    ...
    try {
      ...
      if (user != null) {
        ...
        store.dispatch(EditProfileSuccessfulAction(
            UserFirebase(request.name, user.email ?? '',
                user.uid, photoURL)));
        store.dispatch(updateScreenThunk(NavigateFromEditProfileToHomeScreenAction()));
      }
    } catch (error) {
      print(error);
    }
...
Future<String> uploadFile(File file, FirebaseStorage storage, var userId) async {
